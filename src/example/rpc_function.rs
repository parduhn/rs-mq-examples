...
pub fn start() -> Result<()> {
    ...
    println!("RPC function: Awaiting RPC requests");

    for (i, message) in consumer.receiver().iter().enumerate() {
        match message {
            ConsumerMessage::Delivery(delivery) => {
                let body = String::from_utf8_lossy(&delivery.body);
                println!("RPC function:({:>3}) fib({})", i, body);
                ...
                let (reply_to, corr_id) = match (
                    ...
                    _ => {
                        println!(
                            "RPC function: received delivery without reply_to or correlation_id"
                        );
                ...
                let response = match body.parse() {
                    Ok(n) => format!("{}", fib(n)),
                    Err(_) => "RPC function: invalid input".to_string(),
                };
            ...
            other => {
                println!("RPC function: Consumer ended: {:?}", other);
                break;
            }
        }
    }
